...
launchDate = launchDate.strftime("%d/%m/%Y")
today_cal = open("table.txt", encoding="utf8").read()
...
def cal():
    global launchDate, today_cal

    today_date = datetime.today()
    today_date_format = today_date.strftime("%d/%m/%Y")

    if today_date_format == launchDate:
        print(today_date.weekday())
        return {"calendar": today_cal, "day": 2}
    else:
        today_cal = open("table.txt", encoding="utf8").read()

        return {"calendar": today_cal, "day": today_date.strftime("%d")}


@app.route('/receive', methods=['POST'])
def recv():
    f = open("table.txt", "w", encoding="utf8")
    f.write(request.values["table"])
    f.close()
